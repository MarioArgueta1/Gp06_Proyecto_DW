SELECT c.entity_id AS 'IdClienteN', CONCAT_WS(' ', c.firstname, c.middlename, c.lastname) AS Nombre,
c.email AS 'email', c.dob AS 'FechaDeNacimiento',
CASE 
...
ELSE 'Sin Especificar' END AS 'Pais',
a.city AS 'Ciudad', CONCAT_WS(', ', a.street, a.region) AS 'Direccion'
FROM customer_entity AS c
INNER JOIN customer_address_entity AS a ON a.entity_id = c.entity_id